...
export default async function main(client: GenLayerClient<any>) {
  const filePath = path.resolve(process.cwd(), "contracts/opensource_rewards.py");
  ...
  try {
    ...
    // Deploy with reward amount of 100 tokens per contribution
    const rewardPerContribution = 100;

    const deployTransaction = await client.deployContract({
      code: contractCode,
      args: [rewardPerContribution],
    });
